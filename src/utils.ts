...
/** Default log directory used until setLogDir() is called. */
const DEFAULT_LOG_DIR = join(homedir(), "gemini-images");

/**
 * Resolved log directory. Starts at the default; updated by setLogDir()
 * once config is loaded in main(). Log calls that happen during config
 * loading (before setLogDir is called) fall back to the default safely.
 */
let currentLogDir: string = DEFAULT_LOG_DIR;

/**
 * Set the log directory from the resolved outputDir config value.
 * Call this once after loadConfig() returns, alongside setLogLevel().
 *
 * The resolved path must be absolute. If it resolves outside the user's
 * home directory a warning is logged and the default is kept.
 */
export function setLogDir(dir: string): void {
  const absDir = resolve(dir);
  const homeDir = homedir();
  if (
    !absDir.startsWith(homeDir + "/") &&
    !absDir.startsWith(homeDir + "\\") &&
    absDir !== homeDir
  ) {
    // Write directly to the default dir since currentLogDir hasn't changed yet
    writeToLogFileRaw(
      "WARN",
      [
        `[log] outputDir "${dir}" resolves outside home directory — keeping default log location.`,
      ],
      DEFAULT_LOG_DIR,
    );
    return;
  }
  currentLogDir = absDir;
}

/** Internal: write a log entry to a specific directory (avoids re-entrancy). */
function writeToLogFileRaw(level: string, args: unknown[], logDir: string) {
  try {
    mkdirSync(logDir, { recursive: true });
    const logFile = join(logDir, "gemini-mcp.log");
...
function writeToLogFile(level: string, args: unknown[]) {
  writeToLogFileRaw(level, args, currentLogDir);
}
...
export async function saveImage(opts: SaveImageOptions): Promise<string> {
